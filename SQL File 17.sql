...
BEGIN
    ...
    DECLARE vQuantidade INT;
		SELECT COUNT(*) INTO vQuantidade FROM funcionarios WHERE id = id_funcionario;
...
SELECT busca_funcionario(2) AS FUNCIONARIO;


DELIMITER //
CREATE FUNCTION multiplica (num1 INT, num2 INT)
RETURNS INT
DETERMINISTIC
BEGIN
RETURN num1 * num2;
...
SELECT multiplica(2,8);


-- Calcular o valor de desconto de um produto. Parametros =  valor do produto, quantidade, desconto.
DELIMITER //
CREATE FUNCTION calcula_desconto (valor DECIMAL, quantidade INT, desconto DECIMAL)
...
DELIMITER //
-- Testando a função
SELECT calcula_desconto(450.00, 2, 10) AS RESULTADO;
